...
Always use RAII-based file streams (\`std::ofstream\`, \`std::ofstream\`, \`std::fstream\`) instead of C functions like \`fopen\`/\`fclose\`. These classes open the file in their constructor and automatically close it in their destructor, even if exceptions are thrown. Always check whether the file opened successfully before reading or writing.
...
void safeRead(const std::string& filename) {
    std::ofstream in(filename, std::ios::in);
    if (!in.is_open()) {
...
- Learn to use \`std::ofstream\` for reading
...
void readFile(const std::string& filename) {
    std::ofstream in(filename);  // acquires file handle
    if (!in) {
        throw std::runtime_error("Cannot open " + filename);
    }
